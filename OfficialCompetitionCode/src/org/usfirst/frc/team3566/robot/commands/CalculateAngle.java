	...
	public class CalculateAngle extends InstantCommand {
	    ...
	    protected void initialize() {
			...
			//if too close then don't do this move, may need to change direction later
				...
				Robot.var.rotateTheta=Robot.var.getTheta()-Robot.var.getVectorDegree(p.getX()-Robot.var.x, p.getY()-Robot.var.y);
			System.out.printf("now at x=%.1f y=%.1f theta=%.0f\ngoto x=%.1f y=%.1f\nrotate %.0f go %.1f\n\n", 
					Robot.var.x,Robot.var.y,Robot.var.getTheta(),p.getX(),p.getY(),Robot.var.rotateTheta,Robot.var.distance);
	    }
